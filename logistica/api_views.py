...
from .services import asignar_siguiente_pedido_disponible_a_repartidor, LogisticaService
...
class ConfirmarRecojoView(RepartidorRequiredMixin, APIView):
    def post(self, request, pedido_id):
        ...
        try:
            pedido, asignacion = LogisticaService.confirmar_recojo(
...
class ConfirmarEntregaView(RepartidorRequiredMixin, APIView):
    def post(self, request, pedido_id):
        ...
        if error_response:
            return error_response
        try:
            pedido, asignacion = LogisticaService.confirmar_entrega(
                pedido=asignacion.pedido,
                repartidor=request.user,
                asignacion=asignacion,
                observacion=serializer.validated_data.get("observacion", "")
