...
class PanelNegocioPedidoListView(LoginRequiredMixin, PropietarioNegocioMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if sede:
            ...
            productos_bajo = Producto.objects.filter(sede=sede, stock__lte=3)
            context["productos_bajo_stock"] = productos_bajo
            if productos_bajo.exists():
                nombres = ", ".join([p.nombre for p in productos_bajo])
                messages.warning(
                    self.request, 
                    f"¡Alerta de inventario bajo! Los siguientes productos tienen 3 o menos unidades en stock: {nombres}."
                )
            ...
            try:
                ...
                if pedido_actualizado.estado == EstadoPedido.LISTO_RECOJO:
                    if asignacion:
                        messages.info(
                            request,
                            f"El sistema asigno automaticamente el pedido al repartidor {asignacion.repartidor.username}."
                        )
                    else:
                        messages.warning(
                            request,
                            "El pedido quedo listo para recojo, pero no hay repartidores disponibles en este momento."
                        )
            except TransicionInvalida as e:
                messages.error(request, f"No se pudo actualizar el estado: {str(e)}")
...
def _build_carrito_context(request, form=None):
    ...
    if cupon_aplicado == "NATIVO25" and request.user.is_authenticated:
...
class ProcesarCarritoView(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        except (ProductoSinStock, CarritoVacio, TransicionInvalida) as e:
...
def aplicar_cupon(request):
    if request.method == "POST":
        ...
        if codigo == "NATIVO25":
            has_previous_orders = Pedido.objects.filter(cliente=request.user).exists()
            if not has_previous_orders:
                request.session["cupon_aplicado"] = "NATIVO25"
                messages.success(request, "¡Cupón NATIVO25 aplicado con éxito! Obtienes 25% de descuento en tus productos.")
            else:
                messages.error(request, "El cupón NATIVO25 es válido únicamente para nuevos usuarios en su primera compra.")
        else:
...
def cancelar_pedido(request, pedido_id):
    """Permite al cliente cancelar su pedido solo si está en estado Recibido o Confirmado."""
    pedido = get_object_or_404(Pedido, pk=pedido_id, cliente=request.user)
    estados_cancelables = ['RE', 'CO']
    if pedido.estado in estados_cancelables:
        pedido.estado = 'CN'
        ...
        messages.success(request, f"Tu pedido #{pedido.id} ha sido cancelado. El stock ha sido repuesto.")
    else:
        messages.error(request, "Este pedido ya no puede ser cancelado porque está siendo preparado o en camino.")
    return redirect("pedidos:historial_cliente")
